...
Original file is located at
    https://colab.research.google.com/drive/19IKsKpUx5qUQ8BwQs49R6BtJV03andHb
"""
...
# --- 1. Directorio de Trabajadores y Firmantes (Modificar aquí) ---
...
# 3. Inicializar el estado de la sesión para persistencia de datos (mientras la app está abierta)
if 'orden_data' not in st.session_state:
    st.session_state.orden_data = []

# 4. Control de Numeración Consecutiva (El número se "guarda por memoria" aquí)
if 'siguiente_orden_numero' not in st.session_state:
    # INICIA AQUÍ: Comienza en 929 (el siguiente a tu Orden #928)
    st.session_state.siguiente_orden_numero = 929

if 'siguiente_solicitud_numero' not in st.session_state:
    # Simulación de la numeración 09-10 (Solo un ejemplo, la lógica de incremento es más compleja)
...
st.title("Sistema Automatizado de Órdenes de Mantenimiento (Streamlit) 🛠️")
st.markdown("---")

## 📝 Creación de Nueva Orden de Mantenimiento
...
with st.form(key='orden_form'):
    ...
    with col2:
        st.text_input("Solicitud N°", value=solicitud_actual, disabled=True, help="Número de solicitud consecutivo (Ej: 09-XX)")

    dependencia = st.text_input("Dependencia", value="ÁREA DE MANTENIMIENTO")
    ...
    # --- Solicitud de Material/Ítem ---
    st.markdown("### Detalles de Consumo/Material")
    item_solicitado = st.text_input("Ítem Solicitado (Ej: LÁMPARA DE SOBREPONER DE 18W)")
    ...
    with col_e:
        elaboro = st.selectbox("Elaboró", options=DIRECTORIO_TRABAJADORES["Elaboro"])
    with col_r:
        reviso = st.selectbox("Revisó", options=DIRECTORIO_TRABAJADORES["Reviso"])
    with col_a:
        aprobo = st.selectbox("Aprobó", options=DIRECTORIO_TRABAJADORES["Aprobo"])
    ...
    if submit_button:
        if not motivo_orden or not item_solicitado:
            st.error("Por favor, completa el Motivo de la Orden y el Ítem Solicitado.")
        else:
            nueva_orden = {
                ...
                "Ítem Solicitado": item_solicitado,
